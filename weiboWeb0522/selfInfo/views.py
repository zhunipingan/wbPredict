...
from .models import Observer,MyUser,Frequency,Img
import json
import time
...
def selfCenter(request):
    ...
    my_phone = request.session.get('my_phone', default=None)
    if my_phone is not None:
        imgs = Img.objects.filter(user_phone=my_phone)  # 从数据库中取出所有的图片路径
        my_info = MyUser.objects.get(phone = my_phone)
        return render(request, 'selfInfo/self_center.html',{'my_info':my_info,'img':imgs})
    else:
        return redirect('/self/login')
...
def updateToDB(request):
    ...
    try:
        ...
        if res == 0:
            ...
            MyUser.objects.filter(phone = phone).update(name = name,nick_name = nick_name,self_intro = self_intro)
            ret['status'] = True
            ret['error'] = '更新成功！'
    except Exception as e:
        ret['status'] = False
...
def uploadImg(request): # 图片上传函数
    if request.method == 'POST':
        user_phone = request.session.get('my_phone', default='17863137011')
        img_list = Img.objects.filter(user_phone = user_phone)
        if len(img_list) > 0:
            Img.objects.filter(user_phone = user_phone).delete()
        img = Img(img_url=request.FILES.get('fff'),user_phone = user_phone)
        img.save()
    return redirect('/self/selfCenter')
