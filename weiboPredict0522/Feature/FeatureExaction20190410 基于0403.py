...
def get_blog_time_feature2(weibo_data,userInteractAverage,weibo_total_data):
    ...
    for index, row in weibo_data.iterrows():
        ...
        if last_uid != None and last_uid == uid_i:
            ...
            the_user_data = weibo_total_data[(weibo_total_data.uid == uid_i)]
...
def get_blog_content_feature(weibo_data,is_train_data = 1,num_topics = 5):
    ...
    #0506修改 去除网址
    weibo_data['content'] = weibo_data.content.apply(lambda x:re.sub('(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]','',str(x)))
    # for index,row in weibo_data.iterrows():
    #     pass
    if is_train_data == 1:
        weibo_data = lda.do_train_LDA(weibo_data,num_topics)
    else:
        weibo_data = lda.do_test_LDA(weibo_data,num_topics)
    # print(weibo_data.is_have_emoji)
    return weibo_data


def get_total_weibo_feature(weibo_total_data):
    print('get_total_weibo_feature')
    userGroup = weibo_total_data.groupby('uid')
    print(len(userGroup))
    for uid,group in userGroup:
        time_i = row['time']
        date_i = row['date']
        uid_i = row['uid']
        mid_i = row['mid']
        ...
        i = i + 1
        user_dict = {}
        the_user_data_60day = the_user_data[
            (the_user_data.time <= time_i) & (the_user_data.time >= (time_i - timedelta(days=60)))]
        the_user_data_30day = the_user_data_60day[(the_user_data_60day.time >= (time_i - timedelta(days=30)))]
        the_user_data_15day = the_user_data_30day[(the_user_data_30day.time >= (time_i - timedelta(days=15)))]
        the_user_data_7day = the_user_data_15day[(the_user_data_15day.time >= (time_i - timedelta(days=7)))]
        the_user_data_3day = the_user_data_7day[(the_user_data_7day.time >= (time_i - timedelta(days=3)))]
    ...
    print('get_total_weibo_feature结束')
...
print(weibo_train_data.shape,weibo_predict_data.shape)
weibo_total_data = pd.concat([weibo_train_data,weibo_predict_data],axis=0,ignore_index=True,sort=False)
# print(weibo_total_data)
# get_total_weibo_feature(weibo_total_data)
...
weibo_train_data = get_blog_time_feature2(weibo_train_data,userInteractAverage,weibo_total_data)
endtime2 = datetime.datetime.now()
weibo_train_data.to_csv('../data/weibo_train_data_0523_gbrt5.csv', index=False)
print('blogFeature1用时'+str((endtime2-starttime2).seconds))
starttime3 = datetime.datetime.now()
weibo_predict_data = get_blog_time_feature2(weibo_predict_data,userInteractAverage,weibo_total_data)
weibo_predict_data.to_csv('../data/weibo_predict_data_0523_gbrt5.csv', index=False)
endtime3 = datetime.datetime.now()
print('blogFeature2用时'+str((endtime3-starttime3).seconds))

#3计算额外内容基本特征
num_topics = 5 #代表lda要生成的topic数量
weibo_train_data = get_blog_content_feature(weibo_train_data,is_train_data=1,num_topics=num_topics)
weibo_predict_data = get_blog_content_feature(weibo_predict_data,is_train_data=0,num_topics=num_topics)

#保存微博提取的特征 0522 修改
from sqlalchemy import create_engine
predictors = [x for x in weibo_train_data.columns if x not in ['content_no_stopwords_list','time']]
...
weibo_train_data[predictors].to_sql('weibo_train_data_feature',insert_conn,if_exists='replace',index = False)
print('保存特征数据完成')
...
for i in range(len(target)):
    ...
    score = cross_val_score(rf, weibo_train_data[predictors], weibo_train_data[target[i]], cv=5)#scoring='r2' neg_mean_absolute_error neg_mean_squared_log_error
    print(score,score.mean())
    # 保存model
    # 下面是决策树可视化
    # Estimators = rf.estimators_
    # feature_weight = sorted(zip(map(lambda x: round(x, 4), rf.feature_importances_), predictors), reverse=True)
    # print(feature_weight)
    print("Model save...")
    model_name = "rf"+str(i)+"train_model0523_gbrt5.m"
...
train_result = weibo_train_data.loc[:, ['mid', 'uid', 'time','forward_count', 'comment_count', 'like_count','forward_count_new', 'comment_count_new', 'like_count_new']]
train_result.to_csv(r'D:\graduation project\weiboPredict\train_result0523_gbrt5.csv')

insert_conn = create_engine('mysql+pymysql://' + 'root' + ':' + '123456' + '@' + 'localhost' + ':' + '3306' + '/' + 'weibosimple'+ '?charset=utf8mb4')#+ '?charset=utf8mb4'?charset=utf8mb4
train_result.to_sql('predict_weibo_result0523_gbrt5',insert_conn,if_exists='replace',index = False)
print('保存训练对比数据完成')

endtime4 = datetime.datetime.now()
print('预测用时'+str((endtime4-starttime4).seconds))

#weibo_predict_data.append(weibo_predict_data_old这样不对，old就是原来所有的数据
...
# make_feature(1)
result.to_csv(r'D:\graduation project\weiboPredict\result_0523_gbrt5.csv')

text = open('D:\graduation project\weiboPredict\out0523_rf.txt', 'w')
i = 0
