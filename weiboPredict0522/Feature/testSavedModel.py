...
print(weibo_train_data[predictors].shape)
# for item in target:
#     weibo_predict_data[item] = 0

from sklearn import metrics
from sklearn.metrics import mean_absolute_error
from sklearn.metrics import explained_variance_score,log_loss
...
# X_train, X_test, y_train, y_test = train_test_split(weibo_train_data[predictors], weibo_train_data['like_num'], test_size=0.33, random_state=42)
res = []
dataframe = weibo_train_data.copy()

for i in range(3):
    model_name = "D:\graduation project\weiboPredict\\modelrf"+str(i)+"train_model0402.m"
    ...
    #     print(index)
    #     filename = 'rf0329_' + str(index) + '.pdf'
    ...
    res.append(rf.predict(weibo_train_data[predictors]))
    print(res[i].shape,weibo_train_data[target[i]].shape)
    print('mean_absolute_error',mean_absolute_error(weibo_train_data[target[i]],res[i]))
    print('explained_variance_score',explained_variance_score(weibo_train_data[target[i]],res[i]))
    print('交叉验证',cross_val_score(rf, weibo_train_data[predictors], weibo_train_data[target[i]], cv=3))
    # print('log_loss',log_loss(weibo_train_data['like_count'],res))
    #0409添加
    ...
    # dataframe['true_res'+str(i)] = weibo_train_data[target[i]]
    # dataframe['predict_res'+str(i)] = res[i]
# dataframe.to_csv('../data/testSavedModel_0401.csv')
train_result = weibo_train_data.loc[:, ['mid', 'uid', 'time','content','forward_count', 'comment_count', 'like_count','forward_count_new', 'comment_count_new', 'like_count_new']]
...
#     model_name = "rf"+str(i)+"train_model0329.m"
...
# # make_feature(1)
# result.to_csv(r'D:\graduation project\weiboPredict\result_032102.csv')
#
# text = open('D:\graduation project\weiboPredict\out032103.txt', 'w')
# i = 0
