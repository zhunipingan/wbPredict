...
class WeiboSpider(Spider):
        ...
        for weibo_node in weibo_nodes:
            try:
                ...
                if all_content_link:
                    ...
                    # 0320 应对博文内容不全，因表情而截断
                    extra_content = ''.join(weibo_node.xpath('.//div[1]/text()'))
                    if extra_content != '\xa0':
                        all_content = all_content + (extra_content.split('\xa0')[0])
                ...
                yield Request(url=comment_url, callback=self.parse_comment, meta={'weibo_id': weibo_item['_id']})
    ...
    def parse_weibo_page1(self, response):
        ...
        for weibo_node in weibo_nodes:
            try:
                ...
                weibo_content_node = weibo_node.xpath('.//span[@class="ctt"]')[0]
                # print("weibo_content_node_old",weibo_content_node.xpath('string(.)'))
                # print("weibo_content_node_new", weibo_node.xpath('.//div[1]/text()'))
                ...
                if all_content_link:
                    ...
                    #0320 应对博文内容不全，因表情而截断
                    extra_content = ''.join(weibo_node.xpath('.//div[1]/text()'))
                    # print('微博内容1',weibo_node.xpath('.//div[1]/text()'))

                    if extra_content != '\xa0':
                        all_content = all_content+(extra_content.split('\xa0')[0])
                        # print('微博内容2',all_content)
                    weibo_item['content'] = all_content[0:]
                    yield weibo_item
    ...
    def parse_all_content(self, response):
        ...
        weibo_item['content'] = all_content[1:]
    ...
    def parse_comment(self, response):
        ...
        for comment_node in comment_nodes:
            ...
            comment_item['weibo_id'] = response.meta['weibo_id']
