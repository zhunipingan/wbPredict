...
def query_data_prepare(data):
    # data=pd.read_csv('../data/origion_concat.csv')
    # data=data[data.day>=6]
    data = data.sort_values(by=['uid', 'time']).reset_index(drop=True)
    users = pd.DataFrame(list(set(data['uid'].values)), columns=['uid'])
    l_data = len(users)
    ...
    for i in range(processor):
        ...
        t_data = pd.merge(data, user, on='uid').reset_index(drop=True)
        t_data.to_csv('../data/weibo_data/query_'+str(i)+'.csv',index=False)
        print(len(t_data))

def get_blog_time_featur4():
    res = []
    p = Pool(processor)
    for i in range(processor):
        res.append(p.apply_async(run_get_blog_time_featur4, args=(i,)))
...
def run_get_blog_time_featur4(i):
    weibo_data = pd.read_csv('../data/weibo_data/query_' + str(i) + '.csv')
    weibo_data.time = weibo_data.time.apply(lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S'))
    features = []
    for index, row in weibo_data.iterrows():
        # feature = {}
        # feature['uid'] = row['uid']
        ...
        mid_i = row['mid']
        if index % 1000 == 0:
            print('这是第',i,'process',index,datetime.now())
...
if __name__ == '__main__':
    ...
    query_data_prepare(weibo_train_data)
    gc.collect()
    get_blog_time_featur4()
